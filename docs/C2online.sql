/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     2012-10-3 10:53:51                           */
/*==============================================================*/


drop table if exists c2_admin;

drop table if exists c2_auth;
...
(
   ...
   adm_auth             tinyint(1) not null default 0 comment '1-总管理员 0-一般使用者 2-组长',
   primary key (adm_id)
)
type = MYISAM;
...
(
   ...
   r_status             tinyint not null default 1 comment '1-待发布 2-删除 3-已发布 4-已回滚 5-待审核',
   r_memo               varchar(255) not null,
   adm_id               int not null,
   r_pack_id            int not null,
   primary key (r_id)
)
type = MYISAM;
